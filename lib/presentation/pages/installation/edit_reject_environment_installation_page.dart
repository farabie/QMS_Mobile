...
class EditRejectEnvironmentInstallationPage extends StatefulWidget {
  const EditRejectEnvironmentInstallationPage({super.key});

  @override
  State<EditRejectEnvironmentInstallationPage> createState() =>
      _EditRejectEnvironmentInstallationPageState();
}

class _EditRejectEnvironmentInstallationPageState
    extends State<EditRejectEnvironmentInstallationPage> {
  String? ticketNumber;
  int? stepNumber;
  int? imageLength;
  String? qmsId;
  ...
  void didChangeDependencies() {
    ...
    if (args != null) {
      ...
      reasonRejected = args['reasonRejected'] as String?;
            ...
            TextButton(
              onPressed: () async {
                ...
                try {
                  ...
                  if (response) {
                    // print("Final Project : $finalPhotos");
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          InputWidget.textAreaDisable(
            title: 'Reason Rejected',
            hintText: 'Reason Rejected',
            focusNode: FocusNode(),
            controller: TextEditingController(text: reasonRejected),
          ),
          const Gap(6),
          InputWidget.disable(
                ...
                Expanded(
                  child: GridView.builder(
                    ...
                    itemBuilder: (context, index) {
                      // Add button at the end if we have space for more photos
                      if ((isPhotoUrlMode && index == photoUrls!.length && photoUrls!.length < imageLength) ||
                          (!isPhotoUrlMode && index == documentations.length && documentations.length < imageLength)) {
                      ...
                      // Display image based on active mode
                      if (isPhotoUrlMode) {
                        String imageUrl = photoUrls![index];
                        return buildImageWidget(
                          imageUrl: imageUrl,
                          onRemove: () => removePhotoUrl(index),
                          isNetworkImage: true,
                        );
                      } else {
  ...
  void removeImage(int index) {
    setState(() {
      ...
      if (documentations.isEmpty) {
        // If we've removed all new photos, revert to URL mode if there are URLs
        isPhotoUrlMode = photoUrls!.isNotEmpty;
      }
    });
  }

  // Function to remove a photo URL
  void removePhotoUrl(int index) {
    setState(() {
      if (index < photoUrls!.length) {
        ...
        // Add to deleted photos list for tracking
        deletedPhotoUrls.add(filename);

        // Remove from photoUrls
        photoUrls!.removeAt(index);

        // If all URLs are removed, switch mode based on whether we have new photos
  ...
  // Updated pickImagesFromGallery method
  Future<void> pickImagesFromGallery(int? imageLength) async {
    if (await _requestPermission(
      ...
      print("Before selection - isPhotoUrlMode: $isPhotoUrlMode, photoUrls: ${photoUrls!.length}, docs: ${documentations.length}");
      
      List<XFile>? results = await ImagePicker().pickMultiImage();
      
      if (results.isNotEmpty) {
        // Calculate remaining slots
        int remainingSlots = imageLength! - 
                           (isPhotoUrlMode ? photoUrls!.length : documentations.length);
        ...
        for (XFile file in results) {
          String newFileName = '$qmsInstallationStepId-${increment.toString().padLeft(3, '0')}';
          increment++;
        ...
        // Make all state changes in a single setState call
        setState(() {
          isPhotoUrlMode = false;
          // Only clear photoUrls if we're switching modes from URL to new photos
          if (photoUrls!.isNotEmpty && documentations.isEmpty) {
            photoUrls!.clear();
          }
        ...
        print("After selection - isPhotoUrlMode: $isPhotoUrlMode, photoUrls: ${photoUrls!.length}, docs: ${documentations.length}");
      }
    } else {
      if (mounted) {
        showDialog(
